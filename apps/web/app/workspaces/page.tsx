...
function projectBudget(value: unknown) {
  ...
  return amount > 0 ? `${amount.toLocaleString("ko-KR")}원` : "미설정";
}
  ...
  async function createProject(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const value = await api<{ project_id: string }>("/api/projects", {
        ...
        body: JSON.stringify({ name, mode: "REPLAY" }),
      });
      ...
      <section className="workspace-content-grid">
        <div className="workspace-portfolio-panel">
          ...
          <div className="workspace-table" role="table" aria-label="프로젝트 포트폴리오">
            ...
            {!busy && projects.map((project) => <button className="workspace-table-row" role="row" key={text(project.id)} onClick={() => openProject(project)}><span className="workspace-project-name"><b>{text(project.name, "이름 없는 프로젝트")}</b><small>{text(project.id)}</small></span><span><i className={`workspace-status-dot ${text(project.mode, "LIVE").toLowerCase()}`} />{projectType(project.mode)}</span><span>{text(project.target_finish, "미설정")}</span><span className="workspace-impact-neutral">정상</span><span>{projectBudget(project.extra_budget_krw)}</span></button>)}
          </div>
        </div>
        ...
        <aside className="workspace-side-rail">
          ...
          <form className="create-card workspace-create-panel" id="new-project" onSubmit={createProject}>
            <p className="eyebrow">NEW PROJECT</p>
            <h2>새 프로젝트</h2>
            <p>기준 Excel을 올릴 작업공간을 만듭니다.</p>
            <label>
              프로젝트명
              <input value={name} onChange={(event) => setName(event.target.value)} />
            </label>
            <p className="form-note">개발구매팀 공용 계정에서 관리합니다. 협력사는 계정으로 참여하지 않고 변경 출처·책임 주체·확인 대상으로 기록됩니다.</p>
            <p className="form-note">대응 비용은 프로젝트 생성 시 받지 않고, 실제 변경의 대응안을 비교할 때 입력합니다.</p>
            <button type="submit" disabled={busy}>프로젝트 생성 <span aria-hidden="true">→</span></button>
          </form>
        </aside>
      </section>
    </main>
